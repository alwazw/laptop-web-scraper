...
async def run_live_scrape(limit_per_site=10, save=True, queries=None, sites=None, proxies=None, rotate_proxies=False, randomize_ua=False, stealth_level='basic', db_path=None, proxy_attempts=None):
    ...
    async with async_playwright() as p:
        ...
        for r in unique.values():
            ...
            product_hash = generate_product_hash(src, r.get('cpu_model') or '', r.get('screen_size') or '')
            save_product(product_hash, src, r.get('title'), r.get('cpu_model') or '', r.get('screen_size') or '', is_ram_upgradeable(r.get('title')), True)
            if save:
...
def extract_cpu_from_title(title):
    # Look for common CPU patterns like i7-1185G7, M3 Pro, Ryzen 7 5800U
    m = re.search(r'([iI]\u001f?\d-[\w\d-]+|M\d\s?Pro|Ryzen\s?\d+\s?\w+)', title)
    if m:
        return m.group(0)
...
def main():
    ...
    parser.add_argument('--stealth-level', choices=['basic','aggressive'], default='basic', help='Stealth profile level')
...
Optionally: pip install playwright-stealth""")
            return
        try:
            asyncio.run(run_live_scrape(limit_per_site=args.limit, save=not args.no_save, sites=args.sites, proxies=proxies, rotate_proxies=args.rotate_proxies, randomize_ua=args.randomize_ua, stealth_level=args.stealth_level, db_path=args.db_path))
        except Exception as e:
...
Optionally: pip install playwright-stealth""")
            return
        try:
            asyncio.run(run_live_scrape(limit_per_site=args.limit, save=not args.no_save, sites=args.sites))
        except Exception as e:
            print('Live scrape failed:', e)
